...
public class Force_Interaction{
	...
	public void IL_interact(){
		for(int i=0;i<nOfIntegrins;i++){		
			...
			double k_it=1*sLigand.get(0).Y*(A)/(sLigand.get(0).initialp[2]-sLigand.get(0).substrate[2]);//Y*A/L	
...
protected void CatchBond(){
	...
	for(int i=0;i<nOfLigands;i++){
	  ...
	  sIntegrin.get(sLigand.get(i).integrinBoundNum).kub=2*Math.exp(-0.1*force)+0.000004*Math.exp(0.3*force);	
